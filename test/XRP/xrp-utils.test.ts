...
describe('xrp-drops-conversion', function (): void {
  ...
  it('dropsToXrp() - works with a typical amount', function (): void {
    ...
    const xrp = XrpXrpUtils.dropsToXrp('2000000')
  ...
  it('dropsToXrp() - works with a value ending with a decimal point', function (): void {
    ...
    let xrp = XrpXrpUtils.dropsToXrp('2000000.')
    assert.strictEqual(xrp, '2', '2000000. drops equals 2 XRP')

    xrp = XrpXrpUtils.dropsToXrp('-2000000.')
    assert.strictEqual(xrp, '-2', '-2000000. drops equals -2 XRP')
  })
  ...
  it('dropsToXrp() - works with BigNumber objects', function (): void {
    ...
    let xrp = XrpXrpUtils.dropsToXrp(new BigNumber(2000000))
    assert.strictEqual(xrp, '2', '(BigNumber) 2 million drops equals 2 XRP')

    xrp = XrpXrpUtils.dropsToXrp(new BigNumber(-2000000))
    assert.strictEqual(xrp, '-2', '(BigNumber) -2 million drops equals -2 XRP')

    xrp = XrpXrpUtils.dropsToXrp(new BigNumber(2345678))
    assert.strictEqual(
      xrp,
      '2.345678',
  ...
  it('dropsToXrp() - throws with an amount with too many decimal places', function (): void {
    assert.throws(() => {
      XrpXrpUtils.dropsToXrp('1.2')
    }, /has too many decimal places/u)

    assert.throws(() => {
      XrpXrpUtils.dropsToXrp('0.10')
    }, /has too many decimal places/u)
  })
  ...
  it('dropsToXrp() - throws with an invalid value', function (): void {
    ...
    assert.throws(() => {
      XrpXrpUtils.dropsToXrp('FOO')
    }, /invalid value/u)

    assert.throws(() => {
      XrpXrpUtils.dropsToXrp('1e-7')
    }, /invalid value/u)

    assert.throws(() => {
      XrpXrpUtils.dropsToXrp('2,0')
    }, /invalid value/u)

    assert.throws(() => {
      XrpXrpUtils.dropsToXrp('.')
  ...
  it('xrpToDrops() - works with a typical amount', function (): void {
    ...
    const drops = XrpXrpUtils.xrpToDrops('2')
  ...
  it('xrpToDrops() - works with zero', function (): void {
    ...
    let drops = XrpXrpUtils.xrpToDrops('0')
    assert.strictEqual(drops, '0', '0 XRP equals 0 drops')
    // Negative zero is equivalent to zero
    drops = XrpXrpUtils.xrpToDrops('-0')
    assert.strictEqual(drops, '0', '-0 XRP equals 0 drops')
    drops = XrpXrpUtils.xrpToDrops('0.000000')
    assert.strictEqual(drops, '0', '0.000000 XRP equals 0 drops')
    drops = XrpXrpUtils.xrpToDrops('0.0000000')
  ...
  it('xrpToDrops() - works with a value ending with a decimal point', function (): void {
    ...
    let drops = XrpXrpUtils.xrpToDrops('2.')
    assert.strictEqual(drops, '2000000', '2. XRP equals 2000000 drops')
    drops = XrpXrpUtils.xrpToDrops('-2.')
    assert.strictEqual(drops, '-2000000', '-2. XRP equals -2000000 drops')
  })
  ...
  it('xrpToDrops() - works with BigNumber objects', function (): void {
    ...
    // WHEN converted to drops THEN the conversion is correct and successful
    let drops = XrpXrpUtils.xrpToDrops(new BigNumber(2))
    assert.strictEqual(
      drops,
      '2000000',
      '(BigNumber) 2 XRP equals 2 million drops',
    )
    drops = XrpXrpUtils.xrpToDrops(new BigNumber(-2))
    assert.strictEqual(
      drops,
      '-2000000',
      '(BigNumber) -2 XRP equals -2 million drops',
  ...
  it('xrpToDrops() - throws with an amount with too many decimal places', function (): void {
    ...
    assert.throws(() => {
      XrpXrpUtils.xrpToDrops('1.1234567')
    }, /has too many decimal places/u)
    assert.throws(() => {
      XrpXrpUtils.xrpToDrops('0.0000001')
    }, /has too many decimal places/u)
  })
  ...
  it('xrpToDrops() - throws with an invalid value', function (): void {
    ...
    assert.throws(() => {
      XrpXrpUtils.xrpToDrops('FOO')
    }, /invalid value/u)
    assert.throws(() => {
      XrpXrpUtils.xrpToDrops('1e-7')
    }, /invalid value/u)
    assert.throws(() => {
      XrpXrpUtils.xrpToDrops('2,0')
    }, /invalid value/u)
    assert.throws(() => {
      XrpXrpUtils.xrpToDrops('.')
