...
class PayIdClient {
  ...
  private readonly resultType: PayIdResultType
    ...
    try {
      ...
      if (data?.addresses) {
        switch (this.resultType) {
          ...
          case PayIdResultType.Many: {
            return Promise.reject(PayIDError.unimplemented)
          }
          default: {
            return Promise.reject(
              new PayIDError(PayIDErrorType.Unknown, 'Unknown result type'),
            )
          }
        }
...
export default class SingleNetworkPayIDClient {
   ...
   *                 Man-in-the-Middle attacks. Exposed as an option for testing purposes. Defaults to true.
   */
  constructor(public readonly network: string, useHttps: boolean = true) {
    this.payIdClient = new PayIdClient(network, PayIdResultType.One, useHttps)
  }
   ...
   */
  async addressForPayID(payID: string): Promise<CryptoAddressDetails> {
    return this.payIdClient.addressForPayID(payID)
  }
  ...
  async getInvoice(
    ...
    return this.payIdClient.getInvoice(payID, nonce)
  }
  ...
  async receipt(
    ...
    return this.payIdClient.receipt(payID, invoiceHash, transactionConfirmation)
  }
}
